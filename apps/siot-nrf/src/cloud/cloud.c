...
int cloud_publish(struct device_data *data)
{
	...
	char *const headers[] = {"Connection: close\r\n", NULL};
...
int cert_provision(void)
{
	...
	err = modem_key_mgmt_exists(CONFIG_CLOUD_TLS_SEC_TAG, MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN,
				    &exists);
	if (err) {
	...
	if (exists) {
		mismatch =
			modem_key_mgmt_cmp(CONFIG_CLOUD_TLS_SEC_TAG,
					   MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN, cert, strlen(cert));
		if (!mismatch) {
	...
	err = modem_key_mgmt_write(CONFIG_CLOUD_TLS_SEC_TAG, MODEM_KEY_MGMT_CRED_TYPE_CA_CHAIN,
				   cert, sizeof(cert) - 1);
	if (err) {
