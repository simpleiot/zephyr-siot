...
#define ERROR_MSG(string)	snmp_set_detail(string)
     ...
     * from snmp.c 
     */
    extern uint8_t   sid[];      /* size SID_MAX_LEN */
    int      snmp_errno;
...
#define FIRST_PASS	1
#define	LAST_PASS	2
    uint8_t         *snmp_comstr_parse(uint8_t *, size_t *, uint8_t *,
                                      size_t *, long *);
    uint8_t         *snmp_comstr_build(uint8_t *, size_t *, uint8_t *,
                                      size_t *, long *, size_t);

    int             has_access(uint8_t, int, int, int);
#endif                          /* SNMP_IMPL_H */
