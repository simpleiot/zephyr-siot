...
int snmp_realloc(uint8_t **buf, size_t *buf_len)
{
	...
	if (*buf == NULL) {
		new_buf = k_malloc(new_buf_len);
	} else {
		new_buf = k_realloc(*buf, new_buf_len);
	}
...
void netsnmp_check_definedness(const void *packet, size_t length)
{
	...
	(__VALGRIND_MAJOR__ > 3 || (__VALGRIND_MAJOR__ == 3 && __VALGRIND_MINOR__ >= 6))
	...
	if (RUNNING_ON_VALGRIND) {
		...
		for (i = 0; i < length; ++i) {
			if (VALGRIND_GET_VBITS((const char *)packet + i, &vbits, 1) == 1 && vbits) {
				VALGRIND_PRINTF_BACKTRACE("Undefined: byte %d/%d", i, (int)length);
			}
		}
	}
...
int binary_to_hex(const uint8_t *input, size_t len, char **output)
{
	...
	*output = NULL; /* will alloc new buffer */
	...
	return netsnmp_binary_to_hex((uint8_t **)output, &out_len, 1, input, len);
} /* end binary_to_hex() */
...
{
	int olen = (len / 2) + (len % 2);
	char *s = k_calloc(1, olen ? olen : 1), *op = s;
	const uint8_t *ip = input;
			  ...
			  const char *hex, const char *delim)
{
	...
	while (*cp != '\0') {
		 ...
		 */
		if ((*offset >= *buf_len) && !(allow_realloc && snmp_realloc(buf, buf_len))) {
			return 0;
		}
		*(*buf + *offset) = (uint8_t)subid;
		(*offset)++;
		if (*++cp == '\0') {
			/*
...
char *dump_snmpEngineID(const uint8_t *estring, size_t *estring_len)
{
#define eb(b) (*(esp + b) & 0xff)
	...
	case 2: /* IPv6 address. */
		...
		s += sprintf(s,
			     ...
			     eb(10), eb(11), eb(12), eb(13), eb(14), eb(15));
	...
	case 3: /* MAC address. */
		...
		s += sprintf(s, "%02X:%02X:%02X:%02X:%02X:%02X", eb(0), eb(1), eb(2), eb(3), eb(4),
			     eb(5));
...
/**
 * Returns the difference (in long msec) between the two markers
 *
 * \deprecated Don't use in new code.
 */
long uatime_diff(const_marker_t first, const_marker_t second)
{
	struct timeval diff;
...
/**
 * Returns the difference (in long 1/100th secs) between the two markers
 * (functionally this is what sysUpTime needs)
 *
 * \deprecated Don't use in new code.
 */
long uatime_hdiff(const_marker_t first, const_marker_t second)
{
	struct timeval diff;

	NETSNMP_TIMERSUB((const struct timeval *)second, (const struct timeval *)first, &diff);
	return ((long)diff.tv_sec) * 100 + diff.tv_usec / 10000;
}
...
char *netsnmp_getenv(const char *name)
{
	...
	if (temp == NULL) {
		...
		if (RegOpenKeyExA(HKEY_CURRENT_USER, "SOFTWARE\\Net-SNMP", 0, KEY_QUERY_VALUE,
				  &hKey) == ERROR_SUCCESS) {
			...
			if (RegQueryValueExA(hKey, name, NULL, &key_value_type,
				 ...
				 * terminate the string data in registry is missing one (which is
				 * unlikely).
				 */
				key_value = malloc((sizeof(char) * key_value_size) + sizeof(char));

				if (RegQueryValueExA(hKey, name, NULL, &key_value_type, key_value,
						     &key_value_size) == ERROR_SUCCESS) {
				}
				temp = (char *)key_value;
			}
	...
	/* Next try HKLM */
	if (temp == NULL) {
		if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Net-SNMP", 0, KEY_QUERY_VALUE,
				  &hKey) == ERROR_SUCCESS) {
...
static int big_endian = true;
#else
static int big_endian = false;
#endif
