...
static void _asn_size_err(const char *str, size_t wrongsize, size_t rightsize)
{
	...
	snprintf(ebuf, sizeof(ebuf), "%s size %lu: s/b %lu", str, (unsigned long)wrongsize,
 ...
 *
 */
uint8_t *asn_parse_nlength(uint8_t *pkt, size_t pkt_len, uint32_t *data_len)
{
	int len_len;
 ...
 */
static int _asn_build_header_check(const char *str, const uint8_t *data, size_t datalen,
				   size_t typedlen)
{
	char ebuf[128];
	...
	if (datalen < typedlen) {
		snprintf(ebuf, sizeof(ebuf), "%s: bad header, length too short: %lu < %lu", str,
 ...
 * @return 0 on success 1 on error
 */
static int _asn_realloc_build_header_check(const char *str, uint8_t **pkt, const size_t *pkt_len,
					   size_t typedlen)
{
	char ebuf[128];
 ...
 */
int asn_check_packet(uint8_t *pkt, size_t len)
{
	uint32_t asn_length;
	...
	if (*pkt != (uint8_t)(ASN_SEQUENCE | ASN_CONSTRUCTOR)) {
		return -1; /* wrong type */
	}
...
static int _asn_bitstring_check(const char *str, size_t asn_length, uint8_t datum)
{
	char ebuf[128];

	if (asn_length < 1) {
		snprintf(ebuf, sizeof(ebuf), "%s: length %d too small", str, (int)asn_length);
		ebuf[sizeof(ebuf) - 1] = 0;
		ERROR_MSG(ebuf);
		return 1;
	}
 ...
 */
uint8_t *asn_parse_int(uint8_t *data, size_t *datalength, uint8_t *type, long *intp, size_t intsize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
	 */
	static const char *errpre = "parse int";
	register uint8_t *bufp = data;
	uint32_t asn_length;
	int i;
	union {
		long l;
 ...
 *   of this object (i.e. the start of the next object) Returns NULL on any error
 */
uint8_ *asn_parse_unsigned_int(uint8_t *data, size_t *datalength, uint8_t *type, uint32_t *intp,
			       size_t intsize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_int(uint8_t *data, size_t *datalength, uint8_t type, const long *intp,
		      size_t intsize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
	 ...
	 * integer.
	 */
	mask = ((uint32_t)0x1FF) << ((8 * (sizeof(long) - 1)) - 1);
	/*
	 * mask is 0xFF800000 on a big-endian machine
	 */
	while ((((integer & mask) == 0) || ((integer & mask) == mask)) && intsize > 1) {
		intsize--;
		integer = (uint32_t)integer << 8;
	}
	...
	*datalength -= intsize;
	mask = ((uint32_t)0xFF) << (8 * (sizeof(long) - 1));
	/*
	 * mask is 0xFF000000 if sizeof(long) == 4.
	 */
	while (intsize--) {
		*data++ = (uint8_t)((integer & mask) >> (8 * (sizeof(long) - 1)));
		integer = (uint32_t)integer << 8;
	}
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_unsigned_int(uint8_t *data, size_t *datalength, uint8_t type,
			       const uint32_t *intp, size_t intsize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
	 */
	 ...
	 */
	if ((uint8_t)((integer & mask) >> (8 * (sizeof(long) - 1))) & 0x80) {
		/*
		 * if MSB is set
		 */
		add_null_byte = 1;
		 ...
		 * complement integer. There should be no sequence of 9 consecutive 1's or 0's at
		 * the most significant end of the integer.
		 */
		mask = ((uint32_t)0x1FF) << ((8 * (sizeof(long) - 1)) - 1);
		/*
		 * mask is 0xFF800000 on a big-endian machine
...
uint8_ *asn_parse_string(uint8_t *data, size_t *datalength, uint8_t *type, uint8_t *str,
			 size_t *strlength)
{
	...
	*datalength -= asn_length + (bufp - data);
	...
	DEBUGIF("dumpv_recv")
	{
		uint8_t *buf = (uint8_t *)malloc(1 + asn_length);
...
uint8_ *asn_build_string(uint8_t *data, size_t *datalength, uint8_t type, const uint8_t *str,
			 size_t strlength)
{
	/*
	 * ASN.1 octet string ::= primstring | cmpdstring
	...
	*datalength -= strlength;
	...
	DEBUGIF("dumpv_send")
	{
		uint8_t *buf = (uint8_t *)malloc(1 + strlength);
		size_t l = (buf != NULL) ? (1 + strlength) : 0, ol = 0;

		if (sprint_realloc_asciistring(&buf, &l, &ol, 1, str ? str : (const uint8_t *)"",
					       strlength)) {
 ...
 *
 */
uint8_ *asn_parse_header(uint8_t *data, size_t *datalength, uint8_t *type)
{
 ...
 *
 */
uint8_ *asn_parse_sequence(uint8_t *data, size_t *datalength, uint8_t *type,
...
{ /* error message prefix */
	...
	if (data && (*type != expected_type)) {
		char ebuf[128];
		snprintf(ebuf, sizeof(ebuf), "%s header type %02X: s/b %02X", estr, (uint8_t)*type,
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_header(uint8_t *data, size_t *datalength, uint8_t type, size_t length)
{
	char ebuf[128];
 ...
 *         Returns NULL on any error.
 */
uint8_ *asn_build_sequence(uint8_t *data, size_t *datalength, uint8_t type, size_t length)
{
	static const char *errpre = "build seq";
	char ebuf[128];

	if (*datalength < 4) {
		snprintf(ebuf, sizeof(ebuf), "%s: length %d < 4: PUNT", errpre, (int)*datalength);
		ebuf[sizeof(ebuf) - 1] = 0;
		ERROR_MSG(ebuf);
		return NULL;
	}
 ...
 */
uint8_ *asn_parse_length(uint8_t *data, uint32_t *length)
{
	static const char *errpre = "parse length";
	char ebuf[128];
	register uint8_t lengthbyte;
	...
	if (lengthbyte & ASN_LONG_LEN) {
		...
		if (lengthbyte > sizeof(long)) {
			snprintf(ebuf, sizeof(ebuf), "%s: data length %d > %lu not supported",
				 errpre, lengthbyte, (unsigned long)sizeof(long));
			ebuf[sizeof(ebuf) - 1] = 0;
			ERROR_MSG(ebuf);
			return NULL;
		}
 ...
 *         Returns NULL on any error.
 */
uint8_ *asn_build_length(uint8_t *data, size_t *datalength, size_t length)
{
 ...
 *
 */
uint8_ *asn_parse_objid(uint8_t *data, size_t *datalength, uint8_t *type, oid *objid,
			size_t *objidlength)
{
	...
	register uint32_t subidentifier;
	...
	*datalength -= (int)asn_length + (bufp - data);
	...
	while (length > 0 && (*objidlength)-- > 0) {
		...
		do { /* shift and add in low order 7 bits */
			...
			length--;
		} while ((*(uint8_t *)bufp++ & ASN_BIT8) &&
			 (length > 0)); /* last byte has high bit clear */

		if (length == 0) {
			uint8_t *last_byte = bufp - 1;
 ...
 *           Returns NULL on any error.
 */
uint8_ *asn_build_objid(uint8_t *data, size_t *datalength, uint8_t type, const oid *objid,
			size_t objidlength)
{
	/*
	 * ASN.1 objid ::= 0x06 asnlength subidentifier {subidentifier}*
		...
		*data++ = 0;
		...
		for (i = 1; i < objidlength; i++) {
			...
			for (j = encoded_len - 1; j >= 0; j--) {
				data[j] = (objid_val & 0x7f) | (j == encoded_len - 1 ? 0 : 0x80);
				objid_val >>= 7;
			}
			data += encoded_len;
		}
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_parse_null(uint8_t *data, size_t *datalength, uint8_t *type)
{
	/*
 ...
 *
 */
uint8_ *asn_build_null(uint8_t *data, size_t *datalength, uint8_t type)
{
	/*
	 * ASN.1 null ::= 0x05 0x00
	 */
 ...
 *         Returns NULL on any error.
 */
uint8_ *asn_parse_bitstring(uint8_t *data, size_t *datalength, uint8_t *type, uint8_t *str,
			    size_t *strlength)
{
	/*
 ...
 *         Returns NULL on any error.
 */
uint8_ *asn_build_bitstring(uint8_t *data, size_t *datalength, uint8_t type, const uint8_t *str,
			    size_t strlength)
{
	/*
	...
	if (_asn_bitstring_check(errpre, strlength, (uint8_t)((str) ? *str : 0))) {
		return NULL;
	}
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_parse_unsigned_int64(uint8_t *data, size_t *datalength, uint8_t *type,
				 struct counter64 *cp, size_t countersize)
{
	...
	register uint8_t *bufp = data;
	uint32_t asn_length;
	register uint32_t low = 0, high = 0;
	...
	*type = *bufp++;
	 ...
	 */
	if ((*type == ASN_OPAQUE) && (asn_length <= ASN_OPAQUE_COUNTER64_MX_BER_LEN) &&
	    (*bufp == ASN_OPAQUE_TAG1) &&
	    ((*(bufp + 1) == ASN_OPAQUE_COUNTER64) || (*(bufp + 1) == ASN_OPAQUE_U64))) {
		/*
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_unsigned_int64(uint8_t *data, size_t *datalength, uint8_t type,
				 const struct counter64 *cp, size_t countersize)
{
	...
	{
		uint32_t high = cp->high, low = cp->low;
...
uint8_ *asn_parse_signed_int64(uint8_t *data, size_t *datalength, uint8_t *type,
			       struct counter64 *cp, size_t countersize)
{
	...
	*type = *bufp++;
	 ...
	 */
		...
		snprintf(ebuf, sizeof(ebuf), "%s: wrong type: %d, len %d, buf bytes (%02X,%02X)",
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_signed_int64(uint8_t *data, size_t *datalength, uint8_t type,
			       const struct counter64 *cp, size_t countersize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_parse_float(uint8_t *data, size_t *datalength, uint8_t *type, float *floatp,
			size_t floatsize)
{
 ...
 */
uint8_ *asn_build_float(uint8_t *data, size_t *datalength, uint8_t type, const float *floatp,
			size_t floatsize)
{
	union {
		float floatVal;
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_parse_double(uint8_t *data, size_t *datalength, uint8_t *type, double *doublep,
			 size_t doublesize)
{
 ...
 *          Returns NULL on any error.
 */
uint8_ *asn_build_double(uint8_t *data, size_t *datalength, uint8_t type, const double *doublep,
			 size_t doublesize)
{
	long tmp;
	union {
 ...
 */
int asn_realloc(uint8_t **pkt, size_t *pkt_len)
{
	if (pkt != NULL && pkt_len != NULL) {
		size_t old_pkt_len = *pkt_len;

		DEBUGMSGTL(("asn_realloc", " old_pkt %8p, old_pkt_len %" NETSNMP_PRIz "u\n", *pkt,
			    old_pkt_len));

		if (snmp_realloc(pkt, pkt_len)) {
			DEBUGMSGTL(("asn_realloc", " new_pkt %8p, new_pkt_len %" NETSNMP_PRIz "u\n",
				    *pkt, *pkt_len));
			DEBUGMSGTL(("asn_realloc",
				    " memmove(%8p + %08" NETSNMP_PRIz "x, %8p, %08" NETSNMP_PRIz
				    "x)\n",
 ...
 */
int asn_realloc_rbuild_length(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, size_t length)
{
	static const char *errpre = "build length";
	char ebuf[128];
	int tmp_int;
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_header(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
			      size_t length)
{
	char ebuf[128];
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_int(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
			   const long *intp, size_t intsize)
{
	...
	*(*pkt + *pkt_len - (++*offset)) = (uint8_t)integer;
	integer >>= 8;
	...
	if (asn_realloc_rbuild_header(pkt, pkt_len, offset, r, type, (*offset - start_offset))) {
...
int asn_realloc_rbuild_string(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
			      const uint8_t *str, size_t strlength)
{
		...
		if (_asn_realloc_build_header_check(errpre, pkt, pkt_len, strlength)) {
			...
			DEBUGIF("dumpv_send")
			{
				if (strlength == 0) {
					...
					uint8_t *buf = (uint8_t *)malloc(2 * strlength);
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_unsigned_int(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r,
				    uint8_t type, const uint32_t *intp, size_t intsize)
{
	static const char *errpre = "build uint";
	register uint32_t integer = *intp;
	size_t start_offset = *offset;
...
int asn_realloc_rbuild_sequence(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
				size_t length)
{
 ...
 * @return 1 on success, 0 on error.
 */
static int store_byte(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t byte)
{
 ...
 * @return 1 on success, 0 on error.
 */
static int store_uint32(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint32_t subid)
{
...
int asn_realloc_rbuild_objid(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
...
int asn_realloc_rbuild_null(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type)
{
	/*
	 * ASN.1 null ::= 0x05 0x00
	 */
...
int asn_realloc_rbuild_bitstring(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r,
				 uint8_t type, const uint8_t *str, size_t strlength)
{
	/*
	 * ASN.1 bit string ::= 0x03 asnlength unused {byte}*
	 */
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_unsigned_int64(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r,
				      uint8_t type, const struct counter64 *cp, size_t countersize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
	 */
	register uint32_t low = cp->low, high = cp->high;
	...
	*(*pkt + *pkt_len - (++*offset)) = (uint8_t)low;
	low >>= 8;
	count = 1;
	...
	if (high) {
		...
		*(*pkt + *pkt_len - (++*offset)) = (uint8_t)high;
		high >>= 8;
	...
	if (type == ASN_OPAQUE_COUNTER64) {
		...
		*(*pkt + *pkt_len - (++*offset)) = (uint8_t)intsize;
		...
		*(*pkt + *pkt_len - (++*offset)) = ASN_OPAQUE_TAG1;
		 ...
		 * Put the tag and length for the Opaque wrapper.
		 */
		if (asn_realloc_rbuild_header(pkt, pkt_len, offset, r, ASN_OPAQUE, intsize + 3)) {
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_signed_int64(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r,
				    uint8_t type, const struct counter64 *cp, size_t countersize)
{
	/*
	 * ASN.1 integer ::= 0x02 asnlength byte {byte}*
...
int asn_realloc_rbuild_float(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
			     const float *floatp, size_t floatsize)
{
	...
	*offset += floatsize;
	 ...
	 */
	*(*pkt + *pkt_len - (++*offset)) = (uint8_t)floatsize;
	...
	*(*pkt + *pkt_len - (++*offset)) = ASN_OPAQUE_TAG1;
	 ...
	 * Put the tag and length for the Opaque wrapper.
	 */
	if (asn_realloc_rbuild_header(pkt, pkt_len, offset, r, ASN_OPAQUE, floatsize + 3)) {
		if (_asn_realloc_build_header_check("build float", pkt, pkt_len, floatsize + 3)) {
			return 0;
		} else {
			DEBUGDUMPSETUP("send", (*pkt + *pkt_len - *offset), *offset - start_offset);
			DEBUGMSG(("dumpv_send", "Opaque Float:\t%f\n", *floatp));
...
int asn_realloc_rbuild_double(uint8_t **pkt, size_t *pkt_len, size_t *offset, int r, uint8_t type,
			      const double *doublep, size_t doublesize)
{
	...
	*offset += doublesize;
	 ...
	 * Put the special tag and length (3 bytes).
	 */
	*(*pkt + *pkt_len - (++*offset)) = (uint8_t)doublesize;
	...
	*(*pkt + *pkt_len - (++*offset)) = ASN_OPAQUE_TAG1;
	 ...
	 * Put the tag and length for the Opaque wrapper.
	 */
	if (asn_realloc_rbuild_header(pkt, pkt_len, offset, r, ASN_OPAQUE, doublesize + 3)) {
		if (_asn_realloc_build_header_check("build float", pkt, pkt_len, doublesize + 3)) {
			return 0;
		} else {
			DEBUGDUMPSETUP("send", (*pkt + *pkt_len - *offset), *offset - start_offset);
			DEBUGMSG(("dumpv_send", "  Opaque Double:\t%f\n", *doublep));
