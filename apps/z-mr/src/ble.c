...
static const struct bt_data ad[] Z_GENERIC_SECTION(.rodata) = {
                                 ...
                                 void *buf, uint16_t len, uint16_t offset)
{
    ...
    int ret = points_json_encode(status_points, count,
                               system_status_buffer, sizeof(system_status_buffer));
    ...
    return bt_gatt_attr_read(conn, attr, buf, len, offset,
                            system_status_buffer, strlen(system_status_buffer));
}

static ssize_t read_ats_status(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                              void *buf, uint16_t len, uint16_t offset)
{
    k_mutex_lock(&web_points_lock, K_FOREVER);
    ...
    int ret = points_json_encode(ats_points, count,
                               ats_status_buffer, sizeof(ats_status_buffer));
    k_mutex_unlock(&web_points_lock);
    ...
    return bt_gatt_attr_read(conn, attr, buf, len, offset,
                            ats_status_buffer, strlen(ats_status_buffer));
}
...
BT_GATT_SERVICE_DEFINE(zmr_svc,
    BT_GATT_PRIMARY_SERVICE(BT_UUID_DECLARE_128(SERVICE_UUID_VAL)),
    
    // System Status characteristic
    BT_GATT_CHARACTERISTIC(BT_UUID_DECLARE_128(SYSTEM_STATUS_UUID_VAL),
                          BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                          BT_GATT_PERM_READ,
                          read_system_status, NULL, NULL),
